...
const QuizTakenPage = () => {
  ...
  const handleSubmitQuiz = async () => {
    setSubmitting(true);
    try {
      const formattedAnswer = Object.keys(selectedAnswers).map((questionId) => {
        ...
        return { questionIndex, selectedAnswer: optionIndex };
      });
    ...
    <div className="max-w-4xl mx-auto">
          ...
          >
            <span className="relative z-10 flex items-center justify-center gap-2">
              {submitting ? (
                ...
                <>
                  <CheckCircle2Icon className="w-4 h-4" strokeWidth={2.5} />
                  Submit Quiz
                </>
              )}
            </span>
      ...
      <div className="mt-4 flex items-center justify-center gap-2 flex-wrap">
        {quiz.questions.map((_, index) => {
          const isAnswerQuestion = selectedAnswers.hasOwnProperty(
          ...
          return (
            <button
              ...
              disabled={submitting}
              className={`w-8 h-8 font-semibold text-xs transition-all duration-200
              ${
                isCurrent
